...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'getcurrency',
    'Get the full definition and current state of a currency. Returns the currency\'s configuration (reserves, weights, fees, preallocations, eras) and its latest on-chain state (supply, reserve balances, conversion prices). Use this to check if a currency exists, understand its structure (simple token vs. fractional basket), read current reserve ratios and conversion prices before performing conversions, or look up registration fees (idregistrationfees, idimportfees) before registering identities or defining currencies on a specific chain.',
    {
      chain: z.string().describe('Chain to query (e.g., "VRSC", "vrsctest")'),
        ...
        if (chain) {
          ...
          if (pingResult.running) {
            ...
            return ok({
              ...
              synced: typeof info.connections === 'number' && info.connections > 0,

    ...
    'Estimate the output of converting one currency to another, accounting for pending conversions, fees, and slippage. Does not broadcast a transaction — this is a read-only estimate. Use this before sendcurrency with convertto to preview the expected output. Can estimate a single conversion or an array of conversions using the same basket.',
    {
      ...
      conversion: z.union([
        z.object({
          ...
          via: z.string().optional().describe('Intermediate fractional basket to convert through'),
